...
flags.DEFINE_integer("ghp_approx", None, "Number of nearest distances per node to consider", lower_bound=1)


def eval_from_matrix(train_features, train_labels,
                     ghp_approx=None):
    logging.log(logging.DEBUG, "Start computing distance matrix D with euclidian measure")
    start = time.time()
