...
def eval_from_folder(train_paths, test_paths, path_to_matrix_fn):
    ...
    for i in range(test_iterations):
        if test_iterations == 1:
            ...
            current_samples = min(test_samples,(i+1)*FLAGS.knn_subtest) - current_pos
        ...
        train_iterations = (train_samples + FLAGS.knn_subtrain -1) // FLAGS.knn_subtrain
        d = None
        y_train = None
        for j in range(train_iterations):
            ...
            logging.log(logging.DEBUG, "{}/{} - Start computing distance sub matrix {}/{} with measure='{}'".format(i+1, test_iterations, j+1, train_iterations, FLAGS.knn_measure))
            start = time.time()
            d_j = compute_distance_matrix(train_features, test_features, FLAGS.knn_measure)
            end = time.time()
...
def eval_from_matrices_split(train_features, test_features, train_labels, test_labels,
                             knn_subtest=None, knn_subtrain=None, knn_k=[1], knn_measure="squared_l2"):
    ...
    for i in range(test_iterations):
        ...
        if knn_subtrain is not None:
            ...
            for j in range(train_iterations):
                ...
                logging.log(logging.DEBUG, "{}/{} - Start computing distance sub matrix {}/{} with measure='{}'".format(i+1, test_iterations, j+1, train_iterations, FLAGS.knn_measure))
                start = time.time()
                d_j = compute_distance_matrix(train_features[start_idx:end_idx,:], test_features[current_pos:(current_pos + current_samples),:], FLAGS.knn_measure)
                end = time.time()
                logging.log(logging.DEBUG, "{}/{} - Sub D computed in {} seconds".format(i+1, test_iterations, end - start))
            ...
            logging.log(logging.DEBUG, "{}/{} - Start computing distance matrix with measure='{}'".format(i+1, test_iterations, FLAGS.knn_measure))
            start = time.time()
            d = compute_distance_matrix(train_features, test_features[current_pos:(current_pos + current_samples),:], FLAGS.knn_measure)
            end = time.time()
    ...
    results = {}
    for k, v in vals.items():
        results[KEY_PATTERN.format(knn_measure, k)] = [v, _get_lowerbound(v, k, total_classes)]
...
def eval_from_matrices(train_features, test_features, train_labels, test_labels):
    ...
    start = time.time()
    d = compute_distance_matrix(train_features, test_features, FLAGS.knn_measure)
    end = time.time()
    ...
    for idx, k in enumerate(ks):
        results[KEY_PATTERN.format(FLAGS.knn_measure, k)] = [err[idx], _get_lowerbound(err[idx], k, total_classes)]

    return results


def eval_from_matrix_loo(train_features, train_labels, knn_k=[1], knn_measure="squared_l2"):
    logging.log(logging.DEBUG, "Start computing LOO distance matrix with measure='{}'".format(knn_measure))
    start = time.time()
    d = compute_distance_matrix_loo(train_features, knn_measure)
    end = time.time()
    logging.log(logging.DEBUG, "D computed in {} seconds".format(end - start))
    ...
    for idx, k in enumerate(ks):
        results[KEY_PATTERN.format(knn_measure, k)] = [err[idx], _get_lowerbound(err[idx], k, total_classes)]
